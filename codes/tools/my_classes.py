...
from import_data import (import_hatpro_level1b_daterange_pangaea, import_hatpro_level1b_daterange, 
                        import_mirac_level1b_daterange_pangaea, import_mirac_BRT_RPG_daterange, 
                        import_single_PS122_mosaic_radiosonde_level2, import_single_PS122_mosaic_radiosonde_GRUAN,
                        import_single_mossonde_curM1, import_single_psYYMMDD_wHH_sonde, import_mwr_pro_radiosondes,
                        import_radiosonde_daterange)
from met_tools import convert_rh_to_spechum
from data_tools import (Gband_double_side_band_average, numpydatetime64_to_epochtime, datetime_to_epochtime)
import copy
...
class radiosondes:
    """
        ...
        s_version : str
            ...
            'psYYMMDDwHH', 'level_2', 'GRUAN', 'level_3', 'mwr_pro'. Default: 'level_2' 
            (published by Marion Maturilli)
        single : bool
        ...
        **kwargs:
        ...
        remove_failed : bool
            ...
            for s_version in ['level_2', 'GRUAN']).
    ...
    def __init__(self, path_r, s_version='level_2', single=False, with_wind=False, **kwargs):
        ...
        if single:
            if s_version == 'level_2':
                ...
                sonde_dict = import_single_PS122_mosaic_radiosonde_GRUAN(path_r)
            ...
            if s_version == 'mwr_pro':
                ...
                sonde_dict = import_radiosonde_daterange(path_r, s_version=s_version, with_wind=with_wind, verbose=1, **kwargs)
...
class era5:
    """
        ...
        **kwargs:
        ...
        include_hyd_mets : bool
            Boolean to decide if vertical hydrometeor data (cwc, iwc, rwc and swc), given in kg kg-1, 
            are to be included as attributes and kept in the loaded dataset.
    """
    ...
    def __init__(self, file, **kwargs):
        ...
        if not processed:
            ...
            if 'include_hyd_mets' in kwargs.keys() and kwargs['include_hyd_mets']:
                attribute_list += ['cwc',           # in kg kg-1
                                    'iwc',          # in kg kg-1
                                    'rwc',          # in kg kg-1
                                    'swc']          # in kg kg-1
        ...
        if ('return_DS' in kwargs.keys()) and kwargs['return_DS']:
            ...
            # drop hydrometeors from dataset if not needed (i.e., to save memory):
            if not('include_hyd_mets' in kwargs.keys() and kwargs['include_hyd_mets']):
                for hyd_met in ['cwc', 'iwc', 'rwc', 'swc']:
                    if hyd_met in DS.data_vars:
                        DS = DS.drop(hyd_met)
